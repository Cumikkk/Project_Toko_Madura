...
<script>
    ...
    $(document).ready(() => {
        table_accounting = $('#table-accounting').DataTable({
            ...
            order: [[ 0, "desc" ]],
            ajax: {
                url: "/ajax/datatable/transaction/withdrawal_accounting",
                contentType: "application/json",
                type: "GET",
                data: function(d) {
                    d.filterDateFromAccounting = $('#filterDateFromAccounting').val();
                    d.filterDateToAccounting = $('#filterDateToAccounting').val();
                }
            }
        });
        ...
        $(document).ready(function() {
            ...
            if(table_accounting) {
                table_accounting.on('draw.dt', function() {
                    $.each($('#table-accounting tbody tr'), (i, tr) => {
                        ...
                        if(td) {
                            ...
                            if(actionArea && !actionArea.find('.btn-action').length && actionArea.data('data')) {
                                ...
                                actionArea.append(`<a href="javascript:void(0)" class="btn btn-sm btn-success btn-accounting" data-type="accept" ${dataArray.join(" ")}><i class="fas fa-check"></i></a>`);
                                actionArea.append(`<a href="javascript:void(0)" class="btn btn-sm btn-danger btn-accounting" data-type="reject" ${dataArray.join(" ")}><i class="fas fa-times"></i></a>`);
                    ...
                    $('.btn-accounting').on('click', function(e){
                        ...
                        Swal.fire({
                            ...
                            if(result.isConfirmed) {
                                ...
                                $.post("/ajax/post/transaction/withdrawal_accounting", {note: result.value, type: data.type, 'acc-dpx': data.code}, (resp) => {
