...
// Get Requested Outlet ID
$idOutlet = isset($_GET['id_outlet']) ? (int)$_GET['id_outlet'] : (isset($_GET['outlet_id']) ? (int)$_GET['outlet_id'] : 0);

if ($idOutlet <= 0) {
    if ($role === 'outlet') {
        $resOutId = $db->query("SELECT id_outlet FROM outlet WHERE id_users = {$userId} LIMIT 1");
        if ($resOutId && $resOutId->num_rows > 0) {
            $idOutlet = (int)$resOutId->fetch_assoc()['id_outlet'];
        }
    }
}

if ($idOutlet <= 0) {
    die("ID Outlet tidak valid atau tidak ditentukan.");
}

// Fetch Outlet Details with User & Investor info
$sqlOutlet = "
    SELECT 
        o.id_outlet,
        o.nama_outlet,
        ...
        u_out.nama_lengkap as nama_pengelola,
        u_out.username as username_outlet,
        u_out.no_hp as hp_outlet,
        u_inv.nama_lengkap as nama_investor,
        u_inv.no_hp as hp_investor
    FROM outlet o
    LEFT JOIN users u_out ON o.id_users = u_out.id_users
    LEFT JOIN investor i ON o.id_investor = i.id_investor
    LEFT JOIN users u_inv ON i.id_users = u_inv.id_users
    WHERE o.id_outlet = {$idOutlet}
    LIMIT 1
";

$resOut = $db->query($sqlOutlet);
$outlet = ($resOut && $resOut->num_rows > 0) ? $resOut->fetch_assoc() : null;

if (!$outlet) {
    die("Data outlet tidak ditemukan.");
}
...
// Filter Logic (Rentang Tanggal tgl_mulai & tgl_selesai, bulan, tahun)
...
<head>
    <meta charset="UTF-8">
    <title>Laporan Neraca Sederhana - <?= htmlspecialchars($outlet['nama_outlet']); ?></title>
    <style>
        body {
        ...
        /* Neraca Summary Table (2 Columns Side by Side) */
        .neraca-table {
            width: 100%;
            border-collapse: collapse;
            margin-bottom: 16px;
