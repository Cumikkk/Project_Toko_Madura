                ...
                <script type="text/javascript">
                    $(document).ready(function() {
                        $('.btnStatus').on('click', function() {
                            ...
                            Swal.fire({
                                title: `${typeString} Internal Transfer`,
                                text: `Are you sure you want to ${typeString.toLowerCase()} the internal transfer feature on all user?`,
                                icon: "question",
                                ...
                                if(result.isConfirmed) {
                                    ...
                                    $.post("/ajax/post/internal-transfer/toggle-status", {type: type}, (resp) => {
                ...
                drawCallback : (tbl) => {
                    ...
                    if(!$(`#fbd`).length){
                        $(`input[aria-controls="${$(tbl.nTable).attr('id')}"]`).parent().before(`
                            <label id="fbd">
                                <input type="text" class="filterDate datepicker" ${(prm.dte.length) ? `value="${prm.dte}"` : ``} placeholder="filter by date" style="width : 225px;">
                            </label>
                        `);
                    }
                    ...
                    <div class="card-body">
                        <div class="text-center">
                            ...
                            <div class="alert alert-warning mt-3" role="alert">
                                You don't have any real account yet. Please create a real account to access the internal transfer feature.
                            </div>
                        </div>
                    </div>
                </div>
