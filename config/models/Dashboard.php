...
class Dashboard {
    ...
    public static function getTopByOmzet() {
        $db = Database::connect();
        $sql = "
            SELECT o.id_outlet, o.nama_outlet, mw_out.kecamatan, u_out.alamat_lengkap as alamat_outlet, SUM(l.nominal_omzet) as total_omzet,
                   u_inv.nama_lengkap as nama_investor, mw_inv.kecamatan as kecamatan_investor, u_inv.alamat_lengkap as alamat_investor
    ...
    public static function getRecentRequests() {
        $db = Database::connect();
        $sql = "
            SELECT o.*, u_inv.nama_lengkap as nama_investor, mw_inv.kecamatan as kecamatan_investor, u_inv.alamat_lengkap as alamat_investor
            FROM outlet o
            LEFT JOIN investor inv ON (inv.id_investor = o.id_investor)
            LEFT JOIN users u_inv ON (u_inv.id_users = inv.id_users)
            LEFT JOIN master_wilayah mw_inv ON (mw_inv.id_wilayah = u_inv.id_wilayah)
