                ...
                <form id="form-send-email" enctype="multipart/form-data">
                    <div class="row">
                        <div class="col-md-4 mb-3">
                            ...
                            <input type="date" class="form-control" name="tanggal" id="tanggal" placeholder="Enter date" required>
                        </div>

                        <div class="col-md-4 mb-3">
                            <label class="form-label">Jam <span class="text-danger">*</span></label>
                            <input type="time" class="form-control" name="jam" id="jam" placeholder="Enter time" required>
                        </div>
                        ...
                        <div class="col-md-12 mb-3">
                            ...
                            <input type="file" class="form-control" name="email_content[]" id="email_content" multiple accept=".jpg,.jpeg,.png">
                            <small class="text-muted">Allowed: JPG, PNG. Max size: 10MB per file. Multiple files allowed.</small>
...
$(document).ready(function() {
    $('#email_content').on('change', function() {
    ...
    //     const files = $('#email_content')[0].files;
    ...
    $('#form-send-email').on('submit', function(e) {
        ...
        $.ajax({
            url: '/ajax/post/support/blastemail/create',
            type: 'POST',
            dataType: "json",
            data: formData,
            processData: false,
            ...
            success: function(response) {
                ...
                if(response.success) {
                    ...
                    if ($.fn.DataTable.isDataTable('#tbl_email_history')) {
                        $('#tbl_email_history').DataTable().ajax.reload();
                    }
                }
            },
    ...
    // View Email
    $('#tbl_email_history').on('click', '.btn-view', function() {
        const id = $(this).data('id');
        window.open(`/support/blastemail/detail?id=${id}`, '_blank');
    });
});
</script>
