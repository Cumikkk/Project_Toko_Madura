...
<div class="page-header">
    <div>
        <h2 class="main-content-title tx-24 mg-b-5">Market Analysis</h2>
        <ol class="breadcrumb">
        ...
        <form method="post" id="form-picture">
            <div class="card custom-card">
                <div class="card-body">
                    ...
                    <div class="ql-wrapper ql-wrapper-demo mb-3 text-end">
                        <label class="custom-switch">
                            <span class="custom-switch-description me-2">Protected</span>
                            <input type="checkbox" name="protected" class="custom-switch-input">
                            <span class="custom-switch-indicator"></span>
                        </label>
                    </div>
                </div>
        ...
        $SQL_NEWS = mysqli_query($db, '
            SELECT
                ...
                JSON_OBJECT(
                    ...
                    "edt-protected", tb_blog.BLOG_PROTECTED,
                    "edt-files", tb_blog.BLOG_IMG,
                    "edt-btn", MD5(MD5(tb_blog.ID_BLOG))
                ) AS JSNDT
            FROM tb_blog
            WHERE tb_blog.BLOG_TYPE = 1
            ...
            <div class="card card-aside custom-card">
                ...
                <div class="card-body">
                    <div style="overflow-y: auto;height: 150px;">
                        <a href="javascript:void(0);"><span class="main-content-label tx-16"><?= substr(strip_tags(html_entity_decode($RSLT_NEWS["BLOG_TITLE"])), 0, 35).'...'; ?></span></a>
                        <div class="mt-3">
                    ...
                    <div class="d-flex align-items-center pt-3 mt-auto">
                        <div>
                            ...
                            <strong class="d-block text-muted">Protected: <?= ($RSLT_NEWS["BLOG_PROTECTED"] == -1) ? "<span class='text-success'>Yes</span>" : "<span class='text-danger'>No</span>" ?></strong>
                        </div>
                    ...
                    <div class="modal-body">
                        <div class="card-body">
                            ...
                            <div class="ql-wrapper ql-wrapper-demo mb-3 text-end">
                                <label class="custom-switch">
                                    <span class="custom-switch-description me-2">Protected</span>
                                    <input type="checkbox" name="protected" id="edt-protected" value="0" class="custom-switch-input">
                                    <span class="custom-switch-indicator"></span>
                                </label>
                            </div>
                        </div>
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $('#form-picture').on('submit', function(ev){
            ...
            $.ajax({
                url         : '/ajax/post/news/fundamentals-and-analysis/create',
                type        : 'POST',
                dataType    : 'JSON',
        ...
        $('#form-update').on('submit', function(ev){
            ...
            let data = new FormData(this);
            $.ajax({
                url         : '/ajax/post/news/fundamentals-and-analysis/update',
                type        : 'POST',
                dataType    : 'JSON',
                enctype     : 'multipart/form-data',
                data        : data,
        ...
        $('.dltBtn').on('click', function(e){
            Swal.fire({
                ...
                if(result.isConfirmed) {
                    $.post("/ajax/post/news/fundamentals-and-analysis/delete", {x: $(this).data('value')}, function(resp) {
                        Swal.fire(resp.alert).then(() => {
                            if(resp.success) {
                                location.reload();
                            }
