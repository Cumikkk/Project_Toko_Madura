...
<div class="page-header">
	<div>
		<h2 class="main-content-title tx-24 mg-b-5">Share Rebate</h2>
		<ol class="breadcrumb">
    ...
    $(document).ready(function() {
        ...
        document.querySelectorAll('input[name="datestart"], input[name="dateend"]').forEach((el) => {
            el.addEventListener('change', (event) => {
                ...
                $table.bootstrapTable('refresh', {url: `/ajax/datatable/commision/share/view?start=${dateStart}&end=${dateEnd}`});
            })
        })

        $(function() {
            $table.bootstrapTable({
                url: `/ajax/datatable/commision/share/view?start=${dateStart}&end=${dateEnd}`,
                idField: 'id',
                // showColumns: true,
                columns: [
                    {
                        ...
                        formatter: function(value, row, index) {
                            return (row.pid == 0)
                                ? `<a target="_blank" class="btn btn-sm btn-primary" href="/commision/share/detail/${value}?start=${dateStart}&end=${dateEnd}">Detail</a>`
                                : value;
                        }
                    },
                    {
