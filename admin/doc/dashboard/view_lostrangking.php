...
<script type="text/javascript">
    (function() {
        ...
        let tabel_LostRangking;

        function initLostRangking() {
            if (tabel_LostRangking) {
                tabel_LostRangking.destroy();
            }

            tabel_LostRangking = $('#tabel_LostRangking').DataTable({
                scrollX: true,
                processing: true,
                serverSide: true,
                deferRender: true,
                ...
                info: false,
                ajax: {
                    url: "/ajax/datatable/dashboard/lostrangking/view",
                    contentType: "application/json",
                    type: "GET",
                    error: function(xhr, error, code) {
                        console.error('Lost Ranking fetch error:', error);
                    }
                },
                columns: [
        ...
        if (document.readyState === 'loading') {
            document.addEventListener('DOMContentLoaded', initLostRangking);
        } else {
            // Delay initialization to prevent blocking main thread
            setTimeout(initLostRangking, 100);
        }
    })();
</script>