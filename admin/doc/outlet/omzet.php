...
// Ambil parameter filter bulan & tahun (default: 0 = Semua Bulan & Semua Tahun)
$selectedBulan = isset($_GET['bulan']) ? intval($_GET['bulan']) : 0;
$selectedTahun = isset($_GET['tahun']) ? intval($_GET['tahun']) : 0;

// Query data omzet & keuangan outlet
                            ...
                            <?php if (!empty($outlets)) : ?>
                                ...
                                ?>
                                    <tr data-investor="<?= htmlspecialchars(strtoupper($row['nama_investor'] ?? '')) ?>"
                                        ...
                                        <td class="text-end fw-bold <?= $omzetNom > 0 ? 'text-success' : 'text-muted' ?>">
                                        ...
                                        <td class="text-center">
                                            Rp <?= number_format($tarifNom, 0, ',', '.') ?> / Bln
                                            <?php if ($omzetNom >= 10000000) : ?>
                                                <br><small class="text-success tx-11"><i class="fa fa-arrow-up me-1"></i>Rekomen Naik</small>
                                            <?php endif; ?>
                                        </td>
                                        <td class="text-center">
                                            <div class="action d-flex justify-content-center">
                                                <a href="<?= SystemInfo::app('ADMIN_URL') ?>/outlet/rincian_omzet?id=<?= $row['id_outlet'] ?>&bulan=<?= $selectedBulan ?>&tahun=<?= $selectedTahun ?>" class="btn btn-info btn-sm text-white" title="Rincian Omzet">
...
$(document).ready(function() {
    ...
    $('.filter-select').on('select2:close', function() {
        $(this).next('.select2-container').find('.select2-selection').blur();
    });
    ...
    // Set nilai awal bulan & tahun sesuai parameter GET
    $('#filterBulan').val('<?= $selectedBulan ?>').trigger('change.select2');
    initFilterSelect2('#filterBulan');
    $('#filterTahun').val('<?= $selectedTahun ?>').trigger('change.select2');
    initFilterSelect2('#filterTahun');
    ...
    // ============================================================
    // Event Filter: auto-filter saat change (tanpa tombol submit)
    // ============================================================

    // Filter Bulan & Tahun: reload halaman sesuai pilihan periode
    $('#filterBulan').on('change', function() {
        let bulan = $(this).val();
        let tahun = $('#filterTahun').val();
        window.location.href = adminUrl + '/outlet/omzet?bulan=' + bulan + '&tahun=' + tahun;
    });
    ...
    // Filter Investor: cascade buka Provinsi + auto draw
    ...
    // Filter Provinsi: cascade load Kabupaten + auto draw
    ...
    // Filter Kabupaten: auto draw
    $('#filterKabupaten').on('change', function() {
        if (tableOmzet) tableOmzet.draw();
    });

    // Reset Filter (investor/provinsi/kabupaten saja, bulan+tahun tetap)
