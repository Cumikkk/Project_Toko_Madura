...
if(strtolower($data['deal_type']) == "manual") {
    ...
    $sqlCheckDealDpwd = $db->query("SELECT ID_DPWD FROM tb_dpwd WHERE DPWD_TYPE = ".Dpwd::$typeWithdrawal." AND DPWD_CODE = {$dealId} LIMIT 1");
...
/** check bank source */
$adminBank = Admin::getAdminBank($data['bank-source']);
if(!$adminBank) {
    JsonResponse([
        'success' => false,
        'message' => "Invalid Bank Source",
        'data' => []
    ]);
}

/** Check Bank Receiver */
$userBank = User::myBank($userData['MBR_ID'], $data['bank-receiver']);
if(!$userBank) {
    JsonResponse([
        'success' => false,
...
if(strtolower($data['deal_type']) == "auto") {
    ...
    /** Update Ticket */
    Database::update('tb_dpwd', ['DPWD_CODE' => $withdrawal->ticket], ['ID_DPWD' => $dpwdId]);
}

$db->commit();
Logger::admin_log([
    'admid' => $user['ADM_ID'],
    'module' => "manual_withdrawal",
    'message' => sprintf("Create Manual Withdrawal on %s account %s $%s", $userData['MBR_EMAIL'], $account['ACC_LOGIN'], Helper::formatCurrency($jumlah)),
